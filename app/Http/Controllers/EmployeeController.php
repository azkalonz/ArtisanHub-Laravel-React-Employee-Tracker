...
class EmployeeController extends Controller
{
    public function update(Request $request){
        ...
        if($request->id > 0){
            $user = User::with(['team', 'department', 'shiftSchedule', 'hybridScheduleTeam', 'attendances'])->find($request->id);
        ...
        return User::with(['team', 'department', 'shiftSchedule', 'hybridScheduleTeam', 'attendances'])->find($request->id);
    }

    public function list(Request $request){
        $users = User::with(['team', 'department', 'shiftSchedule', 'hybridScheduleTeam', 'attendances']);
