...
function PtoInfo() {
    ...
    const saveItem = useCallback(() => {
        ...
        setIsSaving(true);
        axios
            .post("update/pto-info/" + selectedItem.id, selectedItem)
            .then(({ data }) => {
                if (data?.id) {
                    const s = [...items];
    ...
    const deleteItem = useCallback(() => {
        ...
        setIsSaving(true);
        axios.delete("delete/pto-info/" + selectedItem.id).then(({ data }) => {
            if (data) {
                const s = [...items];
    ...
    useEffect(() => {
        if (rehydrated) {
            axios.get("list/pto-info").then(async ({ data }) => {
                if (data) {
                    setItems(data);
                }
                setIsLoading(false);
    ...
    return (
        <Grid item xs={12} md={12} lg={12}>
            ...
            <Dialog open={!!selectedItem} onClose={() => setSelectedItem(null)}>
                <DialogTitle>
                    {selectedItem?.id > 0 ? "Update PTO Info" : "Add PTO Info"}
                </DialogTitle>
